...
class TestOrcamentoServicoRepo:
    ...
    def inserir_orcamento_para_teste(
    ...
    def test_obter_orcamento_servico_por_pagina(self, test_db, email_unico, cpf_unico):
        ...
        with sqlite3.connect(test_db) as conn:
            orcamento_servico_pagina_1 = obter_orcamento_servico_por_pagina(
                conn, limit=10, offset=0
            )
            orcamento_servico_pagina_2 = obter_orcamento_servico_por_pagina(
                conn, limit=10, offset=10
            )
            orcamento_servico_pagina_3 = obter_orcamento_servico_por_pagina(
                conn, limit=10, offset=20
            )
        # Assert
        assert (
            len(orcamento_servico_pagina_1) == 10
        ), "A primeira página deveria conter 10 orçamentos de serviços"
        assert (
            len(orcamento_servico_pagina_2) == 5
        ), "A segunda página deveria conter os 5 orçamentos de serviços restantes"
        assert (
            len(orcamento_servico_pagina_3) == 0
        ), "A terceira página não deveria conter nenhum orçamento de serviço"
        # Opcional
        ...
        assert ids_pagina_1.isdisjoint(
            ids_pagina_2
        ), "Os orcamento_servicoes da página 1 não devem se repetir na página 2"
