...
async def alias_cartoes(
    ...
    url = (
        f"/publico/pagamento/cartoes?id_fornecedor={id_fornecedor}"
        if id_fornecedor
        else "/publico/pagamento/cartoes"
    )
...
async def mostrar_meu_plano(request: Request, id_fornecedor: int = 1):
    ...
    return templates.TemplateResponse(
        "fornecedor/planos/minha_assinatura.html",
        {"request": request, "id_fornecedor": id_fornecedor},
    )
...
async def alterar_plano(request: Request, id_plano: int = Form(...)):
    ...
    if not assinatura_ativa:
        ...
        return templates.TemplateResponse(
            "fornecedor/planos/alterar.html",
            {
                "request": request,
                "planos": planos,
                "mensagem": "Você não possui assinatura ativa para alterar.",
            },
        )
    ...
    if (
        plano_novo
        and plano_atual
        and plano_novo.valor_mensal is not None
        and plano_atual.valor_mensal is not None
    ):
        ...
        if plano_novo.valor_mensal > plano_atual.valor_mensal:
            ...
            response = RedirectResponse(
                f"/publico/pagamento/formulario?plano_id={id_plano}&id_fornecedor={id_fornecedor}&tipo=alteracao",
                status_code=303,
            )
            return response
        else:
...
@requer_autenticacao(["fornecedor"])
async def confirmar_cancelamento_plano(
    request: Request, id_fornecedor: int = Form(...)
):
    assinatura_ativa = inscricao_plano_repo.obter_assinatura_ativa_por_fornecedor(
        id_fornecedor
    )
...
async def renovar_plano(request: Request, id_fornecedor: int = Form(...)):
    ...
    if plano_atual:
        ...
        response = RedirectResponse(
            f"/publico/pagamento/formulario?plano_id={plano_atual.id_plano}&id_fornecedor={id_fornecedor}&tipo=renovacao",
            status_code=303,
        )
        return response
...
async def assinar_plano(
    request: Request, plano_id: int = Form(...), id_fornecedor: int = Form(default=1)
):
    assinatura_ativa = inscricao_plano_repo.obter_assinatura_ativa_por_fornecedor(
        id_fornecedor
    )
    if assinatura_ativa:
        planos_disponiveis = plano_repo.obter_plano_por_pagina(
        ...
        return templates.TemplateResponse(
            "fornecedor/planos/assinar.html",
            {
                ...
                "mensagem": "Você já possui uma assinatura ativa. Cancele antes de assinar outro plano.",
            },
        )
    ...
    # Redirecionar para dados de pagamento
    response = RedirectResponse(
        f"/publico/pagamento/formulario?plano_id={plano_id}&id_fornecedor={id_fornecedor}&tipo=assinatura",
        status_code=303,
    )
    return response
...
async def historico_planos(request: Request, id_fornecedor: int = 1):
    ...
    return templates.TemplateResponse(
        "fornecedor/planos/historico_planos.html",
        {"request": request, "historico": historico, "id_fornecedor": id_fornecedor},
    )
