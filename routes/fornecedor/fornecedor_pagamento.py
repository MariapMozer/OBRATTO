...
async def listar_cartoes(request: Request, usuario_logado: Optional[dict] = None):
    ...
    cartoes = cartao_repo.obter_cartoes_fornecedor(fornecedor_id)
    return templates.TemplateResponse(
        "publico/pagamento/meus_cartoes.html",
        {"request": request, "cartoes": cartoes, "id_fornecedor": fornecedor_id},
    )
...
@requer_autenticacao(["fornecedor"])
async def mostrar_adicionar_cartao(
    request: Request, usuario_logado: Optional[dict] = None
):
    assert usuario_logado is not None
    fornecedor_id = usuario_logado["id"]
    return templates.TemplateResponse(
        "publico/pagamento/adicionar_cartao.html",
        {"request": request, "id_fornecedor": fornecedor_id},
    )
...
@requer_autenticacao(["fornecedor"])
async def mostrar_editar_cartao(
    request: Request, id_cartao: int, usuario_logado: Optional[dict] = None
):
    assert usuario_logado is not None
    fornecedor_id = usuario_logado["id"]
    ...
    return templates.TemplateResponse(
        "publico/pagamento/adicionar_cartao.html",
        {"request": request, "cartao": cartao, "id_fornecedor": fornecedor_id},
    )
...
@requer_autenticacao(["fornecedor"])
async def mostrar_confirmar_exclusao(
    request: Request, id_cartao: int, usuario_logado: Optional[dict] = None
):
    assert usuario_logado is not None
    fornecedor_id = usuario_logado["id"]
    ...
    if not cartao or cartao.id_fornecedor != fornecedor_id:
        return RedirectResponse(url="/publico/pagamento/cartoes", status_code=303)
    return templates.TemplateResponse(
        "publico/pagamento/confirmar_exclusao_cartao.html",
        {"request": request, "cartao": cartao},
    )
...
@requer_autenticacao(["fornecedor"])
async def definir_cartao_principal(
    request: Request, usuario_logado: Optional[dict] = None
):
    ...
    if id_cartao_raw is None or isinstance(id_cartao_raw, bytes):
        return RedirectResponse(
            url=f"/publico/pagamento/cartoes?id_fornecedor={fornecedor_id}",
            status_code=303,
        )
    ...
    return RedirectResponse(
        url=f"/publico/pagamento/cartoes?id_fornecedor={fornecedor_id}", status_code=303
    )
