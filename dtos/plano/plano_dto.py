...
class CriarPlanoDTO(BaseDTO):
    ...
    def validar_tipo_plano(cls, v: str) -> str:
        ...
        if v.lower() not in tipos_validos:
            raise ValueError(f'Tipo de plano deve ser um dos seguintes: {", ".join(tipos_validos)}')
        return v.lower()
...
class AlterarPlanoDTO(BaseDTO):
    ...
    nome_plano: Optional[str] = Field(None, min_length=3, max_length=100, description="Nome do plano")
    valor_mensal: Optional[float] = Field(None, gt=0, description="Valor mensal do plano")
    limite_servico: Optional[int] = Field(None, ge=0, description="Limite de serviços permitidos")
    tipo_plano: Optional[str] = Field(None, description="Tipo do plano")
    descricao: Optional[str] = Field(None, min_length=10, max_length=500, description="Descrição do plano")
