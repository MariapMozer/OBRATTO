...
class EditarPerfilAdministradorDTO(BaseDTO):
    ...
    senha_nova: Optional[str] = Field(
        None, min_length=8, description="Nova senha (opcional)"
    )
    confirmar_senha_nova: Optional[str] = Field(
...
class ModerarUsuarioDTO(BaseDTO):
    ...
    nome: Optional[str] = Field(
        None, min_length=2, max_length=100, description="Nome do usuário"
    )
    ...
    motivo_bloqueio: Optional[str] = Field(
        None, max_length=500, description="Motivo do bloqueio"
    )
...
class AprovarSeloDTO(BaseDTO):
    ...
    id_usuario: int = Field(
        ..., gt=0, description="ID do usuário (prestador ou fornecedor)"
    )
    tipo_usuario: str = Field(..., description="Tipo: 'prestador' ou 'fornecedor'")
    aprovar: bool = Field(
        default=True, description="True para aprovar, False para rejeitar"
    )
    observacoes: Optional[str] = Field(
        None, max_length=500, description="Observações sobre a aprovação/rejeição"
    )
...
class ExcluirUsuarioDTO(BaseDTO):
    ...
    tipo_usuario: str = Field(
        ..., description="Tipo: 'prestador', 'fornecedor', 'cliente' ou 'administrador'"
    )
    confirmar_exclusao: bool = Field(
        ..., description="Confirmação obrigatória de exclusão"
    )
    motivo: Optional[str] = Field(
        None, max_length=500, description="Motivo da exclusão"
    )
...
CriarAdministradorDTO.model_config.update(
    {"json_schema_extra": {"example": CriarAdministradorDTO.criar_exemplo_json()}}
)
