...
class CriarAnuncioDTO(BaseDTO):
    ...
    def validar_nome_anuncio(cls, v: str) -> str:
        return validar_texto_obrigatorio(
            v, "Nome do anúncio", min_chars=5, max_chars=150
        )
...
class AlterarAnuncioDTO(BaseDTO):
    ...
    nome_anuncio: Optional[str] = Field(
        None, min_length=5, max_length=150, description="Nome do anúncio"
    )
    descricao: Optional[str] = Field(
        None, min_length=20, max_length=1000, description="Descrição do anúncio"
    )
    ...
    def validar_descricao_alterar(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            return validar_texto_obrigatorio(
                v, "Descrição", min_chars=20, max_chars=1000
            )
        return v
