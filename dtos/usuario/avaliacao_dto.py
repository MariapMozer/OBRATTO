...
class CriarAvaliacaoDTO(BaseDTO):
    ...
    data_avaliacao: date = Field(
        default_factory=date.today, description="Data da avaliação"
    )
    descricao: Optional[str] = Field(
        None, min_length=5, max_length=500, description="Descrição da avaliação"
    )
    nome_avaliador: str = Field(
        ..., min_length=3, max_length=100, description="Nome de quem avaliou"
    )
    nome_avaliado: str = Field(
        ..., min_length=3, max_length=100, description="Nome de quem foi avaliado"
    )
...
class AtualizarAvaliacaoDTO(BaseDTO):
    nota: Optional[int] = Field(
        None, ge=0, le=5, description="Nota da avaliação (0 a 5)"
    )
    data_avaliacao: Optional[date] = Field(None, description="Data da avaliação")
    descricao: Optional[str] = Field(
        None, min_length=5, max_length=500, description="Descrição da avaliação"
    )
    nome_avaliador: Optional[str] = Field(
        None, min_length=3, max_length=100, description="Nome de quem avaliou"
    )
    nome_avaliado: Optional[str] = Field(
        None, min_length=3, max_length=100, description="Nome de quem foi avaliado"
    )
...
CriarAvaliacaoDTO.model_config.update(
    {"json_schema_extra": {"example": CriarAvaliacaoDTO.criar_exemplo_avaliacao_json()}}
)
