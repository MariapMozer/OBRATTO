...
from data.prestador.prestador_sql import (
...
def obter_prestador_por_pagina(conn, limit: int, offset: int) -> list[Prestador]:
    ...
    return [
        Prestador(
            ...
            tipo_usuario=(
                row["tipo_usuario"] if "tipo_usuario" in row.keys() else "prestador"
            ),
            ...
            token_redefinicao=(
                row["token_redefinicao"] if "token_redefinicao" in row.keys() else None
            ),
