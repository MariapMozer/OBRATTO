    ...
    def excluir_cartao(self, id_cartao: int) -> bool:
        ...
        try:
            with open_connection() as con:
                cursor = con.cursor()
                cursor.execute(
                    cartao_sql.DESATIVAR_CARTAO, (datetime.now().isoformat(), id_cartao)
                )
                con.commit()
                return cursor.rowcount > 0
    ...
    def criar_cartao_from_form(
        ...
        try:
            ...
            cartao = CartaoCredito(
                ...
                numero_cartao_criptografado=self.criptografar_numero_cartao(
                    numero_limpo
                ),
